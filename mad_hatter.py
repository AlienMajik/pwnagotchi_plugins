...
class MadHatterUPS:
    ...
    def _max_quick_start(self):
        try:
            self._bus.write_byte_data(KNOWN_I2C_ADDRESSES['max170xx'], MAX_REG_MODE, 0x30)
            time.sleep(0.1)
    ...
    def _gpio_charging(self):
        def read_func():
            return '+' if GPIO.input(self._charging_gpio) == GPIO.LOW else '-'
        try:
...
class MadHatter(plugins.Plugin):
    ...
    __version__ = '1.0.0'
    __license__ = 'GPL3'
    __description__ = 'Universal enhanced plugin for various UPS HATs: Battery indicator, voltage, auto-shutdown, polling, UI customization, error diagnostics, health monitoring, and auto-detection.'
    __defaults__ = {
        'enabled': True,
        'show_voltage': False,
